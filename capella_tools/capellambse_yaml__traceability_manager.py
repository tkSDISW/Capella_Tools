...
class CapellaYAMLTraceabilityHandler:
...
"""
    ...
    def generate_yaml_realized_objects(self):
        """generate YAML content of referenced objects."""
        for ref_obj in self.realized_objects:
    ...
    def generate_traceability_related_objects(self, model, Tstore):
        ...
        for artifact in artifacts:      
            for link in artifact.artifact_links :
                 ...
                 if model_element in self.realized_objects or  model_element in self.primary_objects :
                    #print("Adding Artifact",artifact.name,artifact.uuid)
                    self.realized_objects.append(artifact)
      
    def _track_realized_objects(self, obj):
        ...
        if obj.__class__.__name__ ==  "LogicalComponent" or obj.__class__.__name__ ==  "SystemComponent"  :  
            for comp in obj.components:
                if comp not in self.realized_objects:
                    self.realized_objects.append(comp)
            for port in obj.ports:
                if port not in self.realized_objects:
                    self.realized_objects.append(port)
                for e in port.exchanges:
                    if e not in self.realized_objects:
                        self.realized_objects.append(e)
            for func in obj.allocated_functions:
                if func not in self.realized_objects:
                    self.realized_objects.append(func)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
                for pv in apvg.property_values:
                    if pv not in self.realized_objects:
                        self.realized_objects.append(pv)
            for apv in obj.applied_property_values:
                if apv not in self.realized_objects:
                    self.realized_objects.append(apv)
            for con in obj.constraints:
                if con not in self.realized_objects:
                    self.realized_objects.append(con)
            for sm in obj.state_machines:
                if sm not in self.realized_objects:
                    self.realized_objects.append(sm)
            for req in obj.requirements:
                if req not in self.realized_objects:
                    self.realized_objects.append(req)

        if obj.__class__.__name__ ==  "Entity" :  
            for ent in obj.entities:
                if ent not in self.realized_objects:
                    self.realized_objects.append(ent)
            for act in obj.activities:
                if act not in self.realized_objects:
                    self.realized_objects.append(act)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
                for pv in apvg.property_values:
        ...
        if obj.__class__.__name__  ==  "PhysicalComponent" and obj.nature  ==  "NODE":  
            for dc in getattr(obj, "deployed_components", []):  # Ensure it's iterable
                if hasattr(dc, "name") and hasattr(dc, "uuid"):  # Avoid AttributeError
                    if dc not in self.realized_objects:
                        self.realized_objects.append(dc)
            for comp in obj.components:
                if comp not in self.realized_objects:
                    self.realized_objects.append(comp)
            for physical_port in obj.physical_ports:
                if physical_port not in self.realized_objects:
                    self.realized_objects.append(physical_port)
                for link in physical_port.links:
                    if link not in self.realized_objects:
                        self.realized_objects.append(link)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__  ==  "Requirement" :  
            for rel in obj.relations:
                    if rel not in self.realized_objects:
                        self.realized_objects.append(rel)
        ...
        if obj.__class__.__name__ ==  "LogicalFunction" or obj.__class__.__name__ ==  "SystemFunction" or obj.__class__.__name__ ==  "PhysicalFunction":
            if obj.owner not in self.realized_objects:
                    self.realized_objects.append(obj.owner)
        ...
        if obj.__class__.__name__ ==  "OperationalActivity" :  
            ...
            for ain in obj.inputs:
                if ain not in self.realized_objects:
                    self.realized_objects.append(ain)
            for out in obj.outputs:
                if out not in self.realized_objects:
                    self.realized_objects.append(out)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "OperationalCapability" :  
            for this_obj in obj.includes:
                if this_obj not in self.realized_objects:
                    self.realized_objects.append(this_obj.target)
            for this_obj in obj.extends:
                if this_obj not in self.realized_objects:
                    self.realized_objects.append(this_obj.target)
            for this_obj in obj.involved_entities:
                if this_obj not in self.realized_objects:
                    self.realized_objects.append(this_obj)
        ...
        if  obj.__class__.__name__ ==  "OperationalProcess" :  
            for inv in obj.involved:
                if inv not in self.realized_objects:
                    self.realized_objects.append(inv)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "StateTransition" :  
            for eff in obj.effects:
                if eff not in self.realized_objects:
                    self.realized_objects.append(eff)
            for t in obj.triggers:
                if t not in self.realized_objects:
                    self.realized_objects.append(t)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "State" : 
            for og in obj.outgoing_transitions:
                if og not in self.realized_objects:
                    self.realized_objects.append(og)
            for inc in obj.incoming_transitions:
                if inc not in self.realized_objects:
                    self.realized_objects.append(inc)
            for da in obj.do_activity:
                if da not in self.realized_objects:
                    self.realized_objects.append(da)
            for en in obj.entries:
                if en not in self.realized_objects:
                    self.realized_objects.append(en)
            for ex in obj.exits:
                if ex not in self.realized_objects:
                    self.realized_objects.append(ex)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "StateMachine" :  
            for region in obj.regions:
                for state in region.states:
                    if state not in self.realized_objects:
                        self.realized_objects.append(state)
                for transition in region.transitions:
                    if transition not in self.realized_objects:
                        self.realized_objects.append(transition)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "PropertyValueGroup" :  
            ...
            for pvg in obj.property_value_groups:
                if pvg not in self.realized_objects:
                    self.realized_objects.append(pvg)
        ...
        if obj.__class__.__name__ ==  "FunctionalExchange" :
            for ei in obj.exchange_items:
                if ei not in self.realized_objects:
                    self.realized_objects.append(ei)
            #for fc in obj.involving_functional_chains:
            #    if fc not in self.realized_objects:
            #        self.realized_objects.append(fc)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "Interaction" :
            ...
            #    if op not in self.realized_objects:
            #        self.realized_objects.append(op)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "PhysicalLink" :
            ...
            for obj in obj.exchanges:
                if obj not in self.realized_objects:
                    self.realized_objects.append(obj)
            ...
            if hasattr(obj, "physical_paths"):
                for ppath in obj.physical_paths:
                    if ppath not in self.realized_objects:
                        self.realized_objects.append(ppath)          
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "ComponentExchange" :
            ...
            for afe in obj.allocated_functional_exchanges:
                if afe not in self.realized_objects:
                    self.realized_objects.append(afe)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "ExchangeItemElement" :
            if obj.abstract_type not in self.realized_objects:
                self.realized_objects.append(obj.abstract_type)
            for apvg in obj.applied_property_value_groups:
                if apvg not in self.realized_objects:
                    self.realized_objects.append(apvg)
            for apv in obj.applied_property_values:
        ...
        if obj.__class__.__name__ ==  "Diagram" :
            for node in obj.nodes:
                if node not in self.realized_objects:
                    self.realized_objects.append(node)
        if obj.__class__.__name__ ==  "Part" :
            if obj.type not in self.realized_objects:
                self.realized_objects.append(obj.type)
...
"""     
        ...
        if obj.__class__.__name__ ==  "LogicalComponent" or obj.__class__.__name__ ==  "SystemComponent" :    
            ...
            self._track_realized_objects(obj)
